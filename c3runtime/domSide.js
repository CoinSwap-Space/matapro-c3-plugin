...

{
    ...
    const HANDLER_CLASS = class MetaproDOMHandler extends self.DOMElementHandler {
        constructor(iRuntime) {
            ...
            var stripeScriptEl = document.createElement('script');
            stripeScriptEl.type = 'text/javascript';
            stripeScriptEl.src = 'https://js.stripe.com/v3/';
			document.getElementsByTagName('head')[0].appendChild(stripeScriptEl);
		...
		async _GetSignature(payload) {
			...
			if (provider.provider?.signer?.connection?.bridge === 'https://tst-bridge.metaprotocol.one') {
				signature = await provider.provider.signer.connection.wc.signPersonalMessage([verifyMessage, account]);
			} else {
